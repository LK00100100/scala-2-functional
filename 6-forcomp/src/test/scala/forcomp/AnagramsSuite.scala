...
class AnagramsSuite extends munit.FunSuite {
  ...
  test("sentenceOccurrences: []]") {
    assertEquals(sentenceOccurrences(List()), Nil)
  }
  ...
  test("combinations:  cat") {
    val cat = List(('c', 1), ('a', 2), ('t', 3))

    println(combinations(cat))

    //assertEquals(combinations(cat), List(Nil))
  }
  ...
  test("sentence anagrams: 1 letter test") {
    val sentence = List("nil")
